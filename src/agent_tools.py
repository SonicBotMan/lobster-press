...
def lobster_grep(
    ...
    if search_summaries:
        ...
        for rank_pos, summary in enumerate(summaries[:limit], 1):
            ...
            relevance = (
                (0.7 * rank_score + 0.3 * query_relevance) if use_tfidf_rerank else rank_score
            )
            ...
            results.append(
                {
                    ...
                    + ("..." if len(summary["content"]) > 200 else ""),
...
def lobster_describe(
    db: LobsterDatabase, summary_id: str = None, conversation_id: str = None, depth: int = None
) -> Optional[Dict]:
    """查看摘要详情
...
def lobster_expand(db: LobsterDatabase, summary_id: str, max_depth: int = -1) -> Dict:
    """展开摘要到原始消息
    ...
    try:
        if args.command == "grep":
            ...
            result = lobster_describe(
                db, summary_id=args.summary, conversation_id=args.conversation, depth=args.depth
            )
                ...
                if args.summary:
                    ...
                    if result["kind"] == "leaf":
                        ...
                        for parent in result["parent_summaries"]:
                            print(
                                f"    - {parent['summary_id']}: {parent['kind']} (depth={parent['depth']})"
                            )
                else:
                    # 对话的摘要结构
                    ...
                    for depth in sorted(result["by_depth"].keys()):
                        ...
                        for summary in summaries[:3]:
                            print(
                                f"    - {summary['summary_id']}: {summary['kind']} ({summary['descendant_count']} msgs)"
                            )
                        if len(summaries) > 3:
            ...
            result = lobster_expand(db, summary_id=args.summary, max_depth=args.max_depth)

            if args.json:
                print(json.dumps(result, indent=2, ensure_ascii=False))
            elif args.brief:
                print(f"展开摘要: {result['summary_id']}")
