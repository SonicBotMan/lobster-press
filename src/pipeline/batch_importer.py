...
class BatchImporter:
    ...
    def import_from_json(
        ...
        return self._import_conversations(
            conversations, conversation_id_field, messages_field, batch_size
        )
    ...
    def import_from_csv(
        ...
        # 导入所有对话
        return self._import_conversations(conversations, "conversationId", "messages", batch_size)
    ...
    def _import_messages(self, conversation_id: str, messages: List[Dict], batch_size: int):
        """
        导入消息列表
...
def main():
    ...
    parser = argparse.ArgumentParser(description="LobsterPress v2.5.0 BatchImporter")
    parser.add_argument("input_file", help="输入文件路径（JSON 或 CSV）")
    parser.add_argument(
        "--db", default="lobster_press.db", help="数据库路径（默认: lobster_press.db）"
    )
    parser.add_argument("--format", choices=["json", "csv"], help="输入文件格式（自动检测）")
    parser.add_argument("--batch-size", type=int, default=100, help="批量导入大小（默认: 100）")
    ...
    try:
        ...
        # 导入数据
        if file_format == "json":
            stats = importer.import_from_json(args.input_file, batch_size=args.batch_size)
        else:
            stats = importer.import_from_csv(args.input_file, batch_size=args.batch_size)
