    ...
    def generate_summary(self, intents_data: Dict, max_length: int = 500) -> str:
        ...
        if user_intents:
            ...
            for intent in user_intents[:5]:  # 只显示前 5 个
                repeat_suffix = (
                    f" (重复 {intent.repeat_count} 次)"
                    if hasattr(intent, "repeat_count") and intent.repeat_count > 1
                    else ""
                )
        ...
        if assistant_conclusions:
            ...
            for conclusion in assistant_conclusions[:5]:  # 只显示前 5 个
                repeat_suffix = (
                    f" (重复 {conclusion.repeat_count} 次)"
                    if hasattr(conclusion, "repeat_count") and conclusion.repeat_count > 1
                    else ""
                )
                lines.append(
                    f"  • {conclusion.conclusion_type}: {conclusion.content[:100]}{repeat_suffix}"
                )
