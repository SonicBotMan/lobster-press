...
class ConflictDetector:
    ...
    def __init__(self, use_nli: bool = True, nli_threshold: float = 0.85):
        ...
        if use_nli:
            try:
                ...
                self.nli_model = CrossEncoder("cross-encoder/nli-deberta-v3-small", max_length=256)
                logger.info("ConflictDetector: 使用 NLI 模型（高精度）")
            except ImportError:
                logger.warning("ConflictDetector: sentence-transformers 未安装，降级为规则检测")

    def detect(self, new_message: Dict, existing_notes: List[Dict]) -> List[ConflictResult]:
        """
        检测新消息是否与已有 notes 矛盾。
    ...
    def _check_pair(self, premise: str, hypothesis: str) -> Optional[ConflictResult]:
    ...
    def _check_with_nli(self, premise: str, hypothesis: str) -> Optional[ConflictResult]:
        """使用 NLI 模型检测矛盾（entailment / neutral / contradiction）"""
        try:
            # CrossEncoder 输出 [entailment, neutral, contradiction] 分数
            scores = self.nli_model.predict([(premise, hypothesis)], apply_softmax=True)[0]
    ...
    def _check_with_rules(self, premise: str, hypothesis: str) -> Optional[ConflictResult]:
        """
        规则降级检测：检查否定词 + 关键词共现。
        精度较低，但零依赖。
        ...
        for conflict in conflicts:
            ...
            if new_note_id:
                ...
                semantic_memory.db.cursor.execute(
                    ...
                    WHERE note_id = ?
                """,
                    (new_note_id, datetime.now(timezone.utc).isoformat(), conflict.old_note_id),
                )
                semantic_memory.db.conn.commit()

                logger.info(
                    f"记忆重巩固: [{conflict.old_content[:40]}...] "
