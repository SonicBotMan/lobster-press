...
# -*- coding: utf-8 -*-
"""
LobsterPress v1.4.3 - Bug 修复版本
修复 Issue #75 中的所有 Bug

修复内容：
1. BUG-1 + BUG-2: 将 TFIDFScorer 揕升为实例成员，   - 在 compress() 开始时用全部消息统一构建语料库
   - 在 _score_message() 中复用同一个 scorer 实例
2. BUG-3: 记录 header 的实际行索引
   - 新增 header_line_index 属性
   - 更新 summary_index 计算逻辑
3. 健壮-1: 在 compress() 开始时重置 stats
   - 每次压缩都是独立的统计
4. 健壮-2: 使用实际行数而非解析成功行数
   - original_lines 改为统计文件实际行数
5. 版本号统一为 v1.4.3
...
# 尝试导入 TFIDFScorer，如果失败则使用 fallback
try:
    from tfidf_scorer import TFIDFScorer
    TFIDF_AVAILABLE = True
except ImportError:
    TFIDF_AVAILABLE = False
    print("⚠️ TFIDFScorer 不可用，使用 fallback 评分", file=sys.stderr)
...
class OpenClawSessionParser:
    ...
    def __init__(self):
        ...
        self.header_line_index: Optional[int] = None  # BUG-3 修复：记录 header 的实际行索引
        ...
        for i, line in enumerate(content.strip().split('\n')):
            ...
            try:
                ...
                if obj.get('type') == 'session':
                    self.header = obj
                    self.header_line_index = i  # BUG-3 修复：记录行索引
                # 识别消息
...
class LobsterPressV143:
    """LobsterPress v1.4.3 压缩引擎
    ...
    def __init__(self, 
        ...
        # BUG-1 + BUG-2 修复：TFIDFScorer 作为实例成员
    ...
    def compress(self, content: str) -> Tuple[str, CompressionStats]:
        ...
        Returns:
            (压缩后的 JSONL 内容, 统计信息)
        """
        # 健壮-1 修复：每次压缩都重置 stats
        self.stats = CompressionStats()
        
        parser = OpenClawSessionParser()
        parser.parse_jsonl(content)
        
        # 健壮-2 修复：使用实际行数而非解析成功行数
        actual_lines = len([l for l in content.strip().split('\n') if l.strip()])
        self.stats.original_lines = actual_lines
        ...
        # BUG-1 + BUG-2 修复：在 compress() 开始时统一构建语料库
        if self.tfidf_scorer:
            # 用全部消息构建 IDF 语料库
            self.tfidf_scorer.score_messages(parser.messages)
        ...
        # BUG-3 修复：正确计算 summary_index
        # 1. 收集所有现有索引
        all_existing_indices = set()
        
        # header 的实际行索引
        if parser.header is not None and parser.header_line_index is not None:
            all_existing_indices.add(parser.header_line_index)
        
        # other_lines 的索引
        for idx, _ in parser.other_lines:
            all_existing_indices.add(idx)
        
        # messages 的索引（基于它们在 parser.lines 中的位置）
        for msg in parser.messages:
            # 找到消息在原始 lines 中的索引
            for i, line in enumerate(parser.lines):
                if line is msg or (line.get('id') and line.get('id') == msg.get('id')):
                    all_existing_indices.add(i)
                    break
        
        # 计算 summary_index：找一个不冲突的索引
        if all_existing_indices:
            summary_index = max(all_existing_indices) + 1
        else:
            summary_index = 0
        ...
        if parser.header:
            # 检查是否应该先输出 header
            # 如果 header 是第一行，先输出
            header_idx = parser.header_line_index if parser.header_line_index is not None else 0
            if header_idx == 0 or (parser.other_lines and parser.other_lines[0][0] > header_idx):
                output_lines.append((header_idx, json.dumps(parser.header, ensure_ascii=False)))
        
        # 2. 添加其他非消息行（按原始顺序）
        other_sorted = sorted(parser.other_lines, key=lambda x: x[0])
        for idx, line in other_sorted:
            output_lines.append((idx, json.dumps(line, ensure_ascii=False)))
        
        # 3. 添加摘要消息（在合适的位置）
        if summary:
            summary_msg = self._create_summary_message(summary, self.strategy)
            output_lines.append((summary_index, json.dumps(summary_msg, ensure_ascii=False)))
        ...
        for msg in kept_older:
            ...
            for i, line in enumerate(parser.lines):
                if line is msg or (line.get('id') and line.get('id') == msg.get('id')):
                    output_lines.append((i, json.dumps(msg, ensure_ascii=False)))
                    break
    ...
    def _score_message(self, msg: Dict, parser: OpenClawSessionParser) -> float:
        ...
        BUG-1 + BUG-2 修复：使用统一的 TFIDFScorer 实例
        ...
        Returns:
            重要性分数 (0-1)
        """
        # BUG-1 + BUG-2 修复：使用实例成员的 TFIDFScorer
        if self.tfidf_scorer:
            # 使用已构建语料库的 TFIDFScorer
            scored_msgs = self.tfidf_scorer.score_messages([msg])
            if scored_msgs:
                return scored_msgs[0].final_score / 100.0  # 归一化到 0-1
    ...
    def _create_summary_message(self, summary: str, strategy: str) -> Dict:
        ...
        return {
            ...
            "message": {
                ...
                "content": [{
                    "type": "text",
                    "text": f"[历史摘要 - {strategy} - v1.4.3]\n{summary}"
                }],
                "api": "openai-responses",
                "provider": "openclaw",
                "model": "lobster-press-v143",
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="LobsterPress v1.4.3 - OpenClaw 兼容的压缩引擎 (Bug 修复版本)",
