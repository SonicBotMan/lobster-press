...
class TestMessageWorkflow:
    ...
    def test_message_fts_search(self, db):
        ...
        # 1. 添加消息
        db.add_message(
            conversation_id="conv_1", role="user", content="Python is a programming language"
        )

        db.add_message(
            conversation_id="conv_1", role="assistant", content="JavaScript is also popular"
        )
...
class TestSummaryWorkflow:
    ...
    def test_save_and_retrieve_summary(self, db):
        """测试保存和检索摘要"""
        # 1. 添加消息
        msg_id = db.add_message(conversation_id="conv_1", role="user", content="Test message")
...
class TestCompressionWorkflow:
    ...
    def test_compression_with_sufficient_messages(self, setup):
        ...
        # 2. 执行压缩
        result = compressor.leaf_compact(conversation_id="conv_1", max_tokens=1000)
...
class TestErrorHandling:
    ...
    def test_invalid_conversation_id(self, db):
        """测试无效的对话 ID"""
        # 1. 添加消息到一个对话
        db.add_message(conversation_id="conv_1", role="user", content="Message 1")
...
class TestDatabaseIntegrity:
    ...
    def test_message_count_consistency(self, db):
        ...
        # 1. 添加消息
        for i in range(20):
            db.add_message(conversation_id="conv_1", role="user", content=f"Message {i + 1}")
    ...
    def test_conversation_isolation(self, db):
        ...
        for i in range(5):
            db.add_message(
                conversation_id="conv_1", role="user", content=f"Conv 1 - Message {i + 1}"
            )

        for i in range(3):
            db.add_message(
                conversation_id="conv_2", role="user", content=f"Conv 2 - Message {i + 1}"
            )
