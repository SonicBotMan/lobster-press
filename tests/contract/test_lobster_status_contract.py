...
def test_response_has_required_fields(mcp_server):
    ...
    async def run_test():
        ...
        response = await mcp_server.handle_request(
            {"method": "tools/call", "params": {"name": "lobster_status", "arguments": {}}}
        )
...
def test_response_field_types(mcp_server):
    ...
    async def run_test():
        ...
        assert isinstance(
            result["tier_distribution"], dict
        ), "'tier_distribution' must be a dictionary"
        ...
        if "correction_count" in result:
            assert isinstance(
                result["correction_count"], int
            ), "'correction_count' must be an integer"
        ...
        response = await mcp_server.handle_request(
            {
                ...
                "params": {
                    "name": "lobster_status",
                    "arguments": {"conversation_id": "conv_test_001"},
                },
            }
        )
