...
def test_response_has_required_fields(mcp_server):
    ...
    async def run_test():
        ...
        response = await mcp_server.handle_request(
            {"method": "tools/call", "params": {"name": "lobster_describe", "arguments": {}}}
        )
...
def test_edge_case_nonexistent_summary(mcp_server):
    ...
    async def run_test():
        ...
        response = await mcp_server.handle_request(
            {
                ...
                "params": {
                    "name": "lobster_describe",
                    "arguments": {"summary_id": "summary_nonexistent_12345"},
                },
            }
        )
        ...
        assert result is None or isinstance(
            result, dict
        ), "Should return None or a dictionary even for nonexistent summary"
